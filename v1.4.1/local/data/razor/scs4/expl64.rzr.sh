...
_rsc_rzr_slq_cmd="/usr/bin/squeue"	### SLURM CMD FOR JOB CTRL ###
_rsc_rzr_sli_cmd="/usr/bin/sinfo"	### SLURM CMD FOR NODE CTRL ####
_rsc_rzr_slp_nam="explhir"		### SLURM PARTITION TO CTRL ####
_rsc_rzr_sln_min="6"			### SLURM MIN NODES NUM ###
...
case "$1" in                                                                                                                        
        ...
        ;;                                                                                                                          
        stop|content)
		_rsc_rzr_out_cod_job=$( "$_rsc_rzr_slq_cmd" -w $_rsc_rzr_hostname | wc -l ) 
		_rsc_rzr_out_cod_nod=$( "$_rsc_rzr_sli_cmd" -h -p $_rsc_rzr_slp_nam -o %n 2>/dev/null | wc -l )
		
		[ "$_rsc_rzr_out_cod_job" == "0" ] && [ "$_rsc_rzr_out_cod_nod" -gt "$_rsc_rzr_sln_min" ] && _rsc_rzr_out_cod="0" || _rsc_rzr_out_cod="1"
