...
do
		...
		"h")
			...
			"h")
				...
				echo "	Cyclops monitor change status Options:"
				...
				echo "		group:  wiki format readable"
				echo "		family: excell readable"
...
mng_cyclops()
{
	...
	then
		...
		status)
			...
			echo
			echo -e "Cyclops Monitoring System MODE IS: $( awk -F\; '$1 == "CYC" && $2 == "0001" { print $4 }' $_sensors_sot  )" 
			echo
			echo "Options Status:"
			echo "-----------------------------------"

			_cyc_msg=""
			for _cyc_line in $( cat $_sensors_sot | grep ^CYC )
			do
				_cyc_type=$( echo $_cyc_line | cut -d';' -f1 )
				_cyc_code=$( echo $_cyc_line | cut -d';' -f2 )
				_cyc_code_des=$( grep $_cyc_code $_sensors_sot_codes_file )
				_cyc_msg=$_cyc_msg"\n"$( echo $_cyc_code_des | awk -F\; '{ print $NF ; for ( i=2;i<NF;i++ ) line=line$i";" } END { print line }')
				_cyc_msg=$_cyc_msg"\n"$( echo $_cyc_line | cut -d';' -f3-  )":\n"
			done

			echo -e "${_cyc_msg}" | column -s\; -t | tr ":" "\n" 

			echo 
			echo "Cyclops Modules Cron Active"
			echo "-----------------------------------"
			...
			echo "-----------------------------------"
			$_tool_path/approved/test.productive.env.sh -t pasive -v simple | awk -F\; '{ print "MON DATE:;"$1" "$2 ; print "STATUS IS:;"$4 }' | column -s\; -t
			
		
			_nod_mon_status_tot=$( cat $_type | grep -v \# | wc -l )

			_nod_mon_status_up_lst=$( cat $_type | awk -F\; '$7 == "up" { print $2 }' | tr '\n' ' ' | sed -e 's/ $//' )
			_nod_mon_status_up_num=$( cat $_type | awk -F\; 'BEGIN { _t=0 } $7 == "up" { _t++ } END { print _t }' )
			[ "$_nod_mon_status_tot" == "$_nod_mon_status_up_num" ] && _nod_mon_status_up_lst="all" || _nod_mon_status_up_lst=$( node_group $_nod_mon_status_up_lst )

			_nod_mon_status_dr_lst=$( cat $_type | awk -F\; '$7 == "drain" { print $2 }' | tr '\n' ' ' | sed -e 's/ $//' )
			_nod_mon_status_dr_num=$( cat $_type | awk -F\; 'BEGIN { _t=0 } $7 == "drain" { _t++ } END { print _t }' )
			[ "$_nod_mon_status_tot" == "$_nod_mon_status_dr_num" ] && _nod_mon_status_dr_lst="all" || _nod_mon_status_dr_lst=$( node_group $_nod_mon_status_dr_lst )

			_nod_mon_status_dg_lst=$( cat $_type | awk -F\; '$7 == "diagnose" { print $2 }' | tr '\n' ' ' | sed -e 's/ $//' )
			_nod_mon_status_dg_num=$( cat $_type | awk -F\; 'BEGIN { _t=0 } $7 == "diagnose" { _t++ } END { print _t }' )
			[ "$_nod_mon_status_tot" == "$_nod_mon_status_dg_num" ] && _nod_mon_status_dg_lst="all" || _nod_mon_status_dg_lst=$( node_group $_nod_mon_status_dg_lst )

			_nod_mon_status_lk_lst=$( cat $_type | awk -F\; '$7 == "link" { print $2 }' | tr '\n' ' ' | sed -e 's/ $//' )
			_nod_mon_status_lk_num=$( cat $_type | awk -F\; 'BEGIN { _t=0 } $7 == "link" { _t++ } END { print _t }' )
			[ "$_nod_mon_status_tot" == "$_nod_mon_status_lk_num" ] && _nod_mon_status_lk_lst="all" || _nod_mon_status_lk_lst=$( node_group $_nod_mon_status_lk_lst )

			_nod_mon_status_uk_lst=$( cat $_type | awk -F\; '$7 == "unlink" { print $2 }' | tr '\n' ' ' | sed -e 's/ $//' )
			_nod_mon_status_uk_num=$( cat $_type | awk -F\; 'BEGIN { _t=0 } $7 == "link" { _t++ } END { print _t }' )
			[ "$_nod_mon_status_tot" == "$_nod_mon_status_uk_num" ] && _nod_mon_status_uk_lst="all" || _nod_mon_status_uk_lst=$( node_group $_nod_mon_status_uk_lst )

			_nod_mon_status_rp_lst=$( cat $_type | awk -F\; '$7 == "repair" { print $2 }' | tr '\n' ' ' | sed -e 's/ $//' )
			_nod_mon_status_rp_num=$( cat $_type | awk -F\; 'BEGIN { _t=0 } $7 == "repair" { _t++ } END { print _t }' )
			[ "$_nod_mon_status_tot" == "$_nod_mon_status_rp_num" ] && _nod_mon_status_rp_lst="all" || _nod_mon_status_rp_lst=$( node_group $_nod_mon_status_rp_lst )

			_nod_mon_status_ct_lst=$( cat $_type | awk -F\; '$7 == "content" { print $2 }' | tr '\n' ' ' | sed -e 's/ $//' )
			_nod_mon_status_ct_num=$( cat $_type | awk -F\; 'BEGIN { _t=0 } $7 == "content" { _t++ } END { print _t }' )
			[ "$_nod_mon_status_tot" == "$_nod_mon_status_ct_num" ] && _nod_mon_status_ct_lst="all" || _nod_mon_status_ct_lst=$( node_group $_nod_mon_status_ct_lst )

			echo
			echo "Node Monitoring Status"
			echo "-----------------------------------"
			echo "UP:		$_nod_mon_status_up_num	"$( [ "$_nod_mon_status_up_num" != "0" ] && echo -n "-> $_nod_mon_status_up_lst" )
			echo "DIAGNOSE:	$_nod_mon_status_dg_num	"$( [ "$_nod_mon_status_dg_num" != "0" ] && echo -n "-> $_nod_mon_status_dg_lst" )
			echo "LINKING:	$_nod_mon_status_lk_num "$( [ "$_nod_mon_status_lk_num" != "0" ] && echo -n "-> $_nod_mon_status_lk_lst" )
			echo "UNLINKING:	$_nod_mon_status_uk_num "$( [ "$_nod_mon_status_uk_num" != "0" ] && echo -n "-> $_nod_mon_status_dr_lst" )
			echo "DRAIN:		$_nod_mon_status_dr_num	"$( [ "$_nod_mon_status_dr_num" != "0" ] && echo -n "-> $_nod_mon_status_dr_lst" )
			echo "REPAIR:		$_nod_mon_status_rp_num	"$( [ "$_nod_mon_status_rp_num" != "0" ] && echo -n "-> $_nod_mon_status_rp_lst" )
			echo "CONTENT:	$_nod_mon_status_ct_num	"$( [ "$_nod_mon_status_ct_num" != "0" ] && echo -n "-> $_nod_mon_status_ct_lst" )
			echo "TOTAL NODES:	$_nod_mon_status_tot"
			echo
